    ...
    model(0)
{
    ...
    ui->ELYAAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->BittrexELYALabel->setTextFormat(Qt::RichText);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "ELYA(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "ELYA(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    /*OrderBook Table Init*/

    /*Market History Table Init*/
    ui->MarketHistoryTable->setColumnCount(5);
    ui->MarketHistoryTable->verticalHeader()->setVisible(false);
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(ELYA)"<<"TOTAL COST(BTC");
...
void tradingDialog::UpdaterFunction(){
    //ELYAst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://bittrex.com/api/v1.1/public/GetMarketSummary?market=btc-ELYA");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://bittrex.com/api/v1.1/public/getorderbook?market=BTC-ELYA&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://bittrex.com/api/v1.1/public/getmarkethistory?market=BTC-ELYA&count=100");
      return Response;
}
...
QString tradingDialog::BuyELYA(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://bittrex.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-ELYA&quantity=";
...
QString tradingDialog::SellELYA(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://bittrex.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-ELYA";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=ELYA";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-ELYA&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>ELYA Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> ELYA");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    ui->ELYASupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(ELYASupply,'i',8) + "</span><b> ELYA</b>");
    ...
    ui->ELYADemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(ELYADemand,'i',8) + "</span><b> ELYA</b>");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
                case 0:    //buy tab is active
                    ...
                    Response2 = GetBalance("ELYA");
                    Response3 = GetOrderBook();

                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BtcAvailableLabel, *ui->ELYAAvailableLabel, Response, Response2);
                    }
                ...
                case 1: //Cross send tab active
                    Response = GetBalance("ELYA");
                    Response2 = GetBalance("BTC");
                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BittrexELYALabel, *ui->BittrexBTCLabel, Response, Response2);
                    }
                ...
                case 5://show balance tab
                    ...
                    Response = GetBalance("ELYA");
                    if(Response.size() > 0 && Response != "Error"){
                        DisplayBalance(*ui->ELYABalanceLabel,*ui->ELYAAvailableLabel_2,*ui->ELYAPendingLabel, QString::fromUtf8("ELYA"),Response);
                    }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputELYA->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("ELYA");
    ...
    double AvailableELYA = BalanceObject["Available"].toDouble();
...
{
    //calculate amount of BTC that can be gained from selling ELYA available balance
    QString responseA = GetBalance("ELYA");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailableELYA = ResultObject["Available"].toDouble();

    ui->UnitsInputELYA->setText(str.number(AvailableELYA,'i',8));
}
...
{
    double Quantity = ui->BittrexELYALabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void tradingDialog::on_Withdraw_Max_Amount_clicked()
{
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableELYA,'i',8));
}
...
void tradingDialog::on_BuyELYA_clicked()
{
    double Rate;
    double Quantity;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  BuyELYA(Order,Quantity,Rate);
...
void tradingDialog::on_SellELYABTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputELYA->text().toDouble();
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  SellELYA(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
    ...
    if(!ctx.isValid())
    {
        ...
        foreach (const QJsonValue & value, BuyArray)
        {
            ...
            if ( ((Quantity / x) - y) > 0 )
            {
                ...
                QString SellResponse = SellELYA(Order,y,x);
                ...
                QString SellResponse = SellELYA(Order,(Quantity / x),x);
                QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
                QJsonObject SellResponseObject = SelljsonResponse.object();                              //get json obj
                ...
                if (SellResponseObject["success"].toBool() == false){
                    ...
                    if (ResponseObject["success"].toBool() == false){
                        ...
                        if (ResponseObject["success"].toBool() == false){
                            ...
                            QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" ELYA for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                        }
                    } else if (ResponseObject["success"].toBool() == true){
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" ELYA for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
...
void tradingDialog::on_UnitsInputELYA_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
