...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeElyaCoinV2Amount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeElyaCoinV2Amount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeElyaCoinV2Amount = strAnonymizeElyaCoinV2Amount.remove(strAnonymizeElyaCoinV2Amount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeElyaCoinV2Amount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeElyaCoinV2Amount*COIN) nMaxToAnonymize = nAnonymizeElyaCoinV2Amount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeElyaCoinV2Amount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeElyaCoinV2Amount));
        strAnonymizeElyaCoinV2Amount = strAnonymizeElyaCoinV2Amount.remove(strAnonymizeElyaCoinV2Amount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeElyaCoinV2Amount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeElyaCoinV2Amount == 0){
