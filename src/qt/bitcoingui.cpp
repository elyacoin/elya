...
{
	resize(900, 520);
	setWindowTitle(tr("ElyaCoinV2") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
	setObjectName("ElyaCoinV2");
	setStyleSheet("#ElyaCoinV2 { background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 1.4, stop: 0 #dedede, stop: 1 #efefef);  }");
	// Accept D&D of URIs
	setAcceptDrops(true);
...
void BitcoinGUI::createActions()
{
	...
	sendCoinsAction->setToolTip(tr("Send coins to a ElyaCoinV2 address"));
	...
	aboutAction = new QAction(QIcon(":/icons/bitcoin1"), tr("&About ElyaCoinV2"), this);
	aboutAction->setToolTip(tr("Show information about ElyaCoinV2"));
	aboutAction->setMenuRole(QAction::AboutRole);
	aboutQtAction = new QAction(QIcon(":/qt-project.org/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
	aboutQtAction->setToolTip(tr("Show information about Qt"));
	aboutQtAction->setMenuRole(QAction::AboutQtRole);
	optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
	optionsAction->setToolTip(tr("Modify configuration options for ElyaCoinV2"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
	...
	if (clientModel)
	{
		...
		if (clientModel->isTestNet())
		{
			...
			if (trayIcon)
			{
				trayIcon->setToolTip(tr("ElyaCoinV2 client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
	...
	trayIcon->setToolTip(tr("ElyaCoinV2 client"));
...
void BitcoinGUI::setNumConnections(int count)
{
	...
	labelConnectionsIcon->setToolTip(tr("%n active connection(s) to ElyaCoinV2 network", "", count));
}
...
{
	QString strTitle = tr("ElyaCoinV2") + " - ";
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
	if (event->mimeData()->hasUrls())
	{
		...
		if (nValidUrisFound)
			...
			notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid ElyaCoinV2 address or malformed URI parameters."));
	}
