...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("ElyaCoinV2"))
        return false;
...
{
    // Convert ElyaCoinV2:// to ElyaCoinV2:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ElyaCoinV2://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "ElyaCoinV2:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ElyaCoinV2.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for ElyaCoinV2.lnk
...
{
    return GetAutostartDir() / "ElyaCoinV2.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a ElyaCoinV2.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=ElyaCoinV2\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ElyaCoinV2 app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add ElyaCoinV2 app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("ElyaCoinV2-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  ElyaCoinV2-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("ElyaCoinV2-Qt"));
