...
void Shutdown()
{
    ...
    RenameThread("ElyaCoinV2-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: ElyaCoinV2.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: ElyaCoinV2d.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 14285)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 14286)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeElyaCoinV2amount=<n> " + _("Keep N ElyaCoinV2 anonymized (default: 0)") + "\n";
    ...
    strUsage += "  -stakethreshold=<n> " + _("This will set the output size of your stakes to never be below this number (default: 200771)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ElyaCoinV2 is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ElyaCoinV2 is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("ElyaCoinV2 version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "ElyaCoinV2 server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ElyaCoinV2") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart ElyaCoinV2 to complete") << "\n";
    ...
    nAnonymizeElyaCoinV2Amount = GetArg("-anonymizeElyaCoinV2amount", 0);
    if(nAnonymizeElyaCoinV2Amount > 999999) nAnonymizeElyaCoinV2Amount = 999999;
    if(nAnonymizeElyaCoinV2Amount < 2) nAnonymizeElyaCoinV2Amount = 2;
    ...
    LogPrintf("Anonymize ElyaCoinV2 Amount %d\n", nAnonymizeElyaCoinV2Amount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1ELYA+1000 == (.1ELYA+100)*10
       10ELYA+10000 == (1ELYA+1000)*10
    */
