...
int nAnonymizeElyaCoinV2Amount = 25000;
...
#else
    const char* pszModule = "ElyaCoinV2";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ElyaCoinV2
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ElyaCoinV2
    // Mac: ~/Library/Application Support/ElyaCoinV2
    // Unix: ~/.ElyaCoinV2
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ElyaCoinV2";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ElyaCoinV2.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ElyaCoinV2d.pid"));
