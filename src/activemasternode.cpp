...
//
// Bootup the masternode, look for a 500 ELYA input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in ElyaCoinV2.conf */
...
{
    CElyaCoinV2Address address(collateralAddress);
